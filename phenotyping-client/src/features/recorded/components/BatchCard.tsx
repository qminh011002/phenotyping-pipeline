...
const batchCardDateFormatter = new Intl.DateTimeFormat(undefined, {
    month: 'short',
    day: 'numeric',
    year: 'numeric',
});
const batchCardTimeFormatter = new Intl.DateTimeFormat(undefined, {
    hour: '2-digit',
    minute: '2-digit',
});

function formatDate(isoString: string): string {
    return batchCardDateFormatter.format(new Date(isoString));
}

function formatTime(isoString: string): string {
    return batchCardTimeFormatter.format(new Date(isoString));
}
                ...
                <AlertDialogContent onClick={(e) => e.stopPropagation()}>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            ...
                            {batch.total_image_count !== 1 ? 's' : ''}. This action cannot be
                            undone.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
...
function ConfidenceChip({ confidencePct }: { confidencePct: number | null }) {
    ...
    return (
        ...
        >
            ...
            <span>
                <span className="font-semibold tabular-nums text-foreground">{confidencePct}%</span>{' '}
                Conf
            </span>
        </span>
    );
...
function BatchCardImpl({ batch, onDelete }: BatchCardProps) {
    ...
    const confidencePct =
        batch.avg_confidence != null ? Math.round(batch.avg_confidence * 100) : null;
            ...
            <div className="mt-4 flex min-w-0 shrink-0 items-start px-3.5 pb-3.5">
                <div className="flex min-w-0 flex-1 flex-wrap items-center gap-1.5 overflow-hidden">
                    <StatChip
                        icon={ImageIcon}
                        label="Images"
                        value={batch.total_image_count.toLocaleString()}
                    />
