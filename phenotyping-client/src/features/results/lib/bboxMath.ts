...
export function hitTestHandle(
    ...
    for (const [pos, { x, y }] of Object.entries(handles) as [
        HandlePos,
        { x: number; y: number },
    ][]) {
 ...
 */
export function hitTestBody(annotations: BBox[], px: number, py: number): number | null {
...
export function boxesEqual(a: BBox[], b: BBox[]): boolean {
    ...
    for (let i = 0; i < a.length; i++) {
        ...
        if (
            aBox.bbox[0] !== bBox.bbox[0] ||
            aBox.bbox[1] !== bBox.bbox[1] ||
            aBox.bbox[2] !== bBox.bbox[2] ||
            aBox.bbox[3] !== bBox.bbox[3]
        )
            return false;
