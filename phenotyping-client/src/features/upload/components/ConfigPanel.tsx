...
export function ConfigPanel({ open, onOpenChange, onSaved }: ConfigPanelProps) {
    ...
    function validateSingle(key: string, value: unknown): Record<string, string> {
        ...
        if (key === 'confidence_threshold') {
            const v = value as number;
            if (v < 0.01 || v > 1.0)
                return { confidence_threshold: 'Must be between 0.01 and 1.0' };
        }
        if (key === 'min_box_area') {
            const v = value as number;
            if (!Number.isInteger(v) || v < 1)
                return { min_box_area: 'Must be a positive integer' };
        }
        if (key === 'edge_margin') {
            const v = value as number;
            if (!Number.isInteger(v) || v < 0)
                return { edge_margin: 'Must be a non-negative integer' };
        }
                        ...
                        <div className="flex flex-col gap-5">
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    <Slider
                                        ...
                                        onValueChange={([v]) =>
                                            updateField('confidence_threshold', v)
                                        }
                                        disabled={saving}
                                        className="flex-1"
                                    />
                            ...
                            >
                                <Select
                                    ...
                                    onValueChange={(v) =>
                                        updateField('dedup_mode', v as 'center_zone' | 'edge_nms')
                                    }
                                    disabled={saving}
                                >
                            ...
                            {showEdgeNmsFields && (
                                <div className="space-y-4 rounded-md border border-border bg-muted/30 p-4">
                                    ...
                                    <LabeledField
                                        ...
                                        error={
                                            validationErrors.edge_margin ?? fieldErrors.edge_margin
                                        }
                                    >
                                        <NumberField
                                            id="edge-margin"
                                    ...
                                    <LabeledField
                                        ...
                                        error={
                                            validationErrors.nms_iou_threshold ??
                                            fieldErrors.nms_iou_threshold
                                        }
                                    >
                                        <div className="flex items-center gap-3">
                                            <Slider
